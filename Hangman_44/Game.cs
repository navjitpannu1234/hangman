...
namespace Hangman_44
{
    ...
    public class Game : Activity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            LoadWords_from_Dictionnary();


            btnNewGame = FindViewById<Button>(Resource.Id.btnNewGame);
            txtGuessWord = FindViewById<TextView>(Resource.Id.txtGuessWord);
            ...
            All_Disable_Buttons();
            Hangman_image = FindViewById<ImageView>(Resource.Id.Hangman_image);
            Load_blank_Image();
        ...
        private void Letter_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < HiddenWord.Length; i++)
            {
                ...
                if (letter == wordToGuess[i].ToString())
                {
                    ...
                    txtGuessWord.Text = string.Join(" ", HiddenWord);
        ...
        private void LoadWords_from_Dictionnary()
        {

            // Open the textfile
            Stream myStream = Assets.Open("Dictionary.txt");
        ...
        private void GuessFailed()
        {
            switch (GuessesLeft)
            {
                case 7:
                    ...
                    Hangman_image.SetImageResource(Resource.Drawable.Fail1);

                    break;
                case 6:
                    Hangman_image.SetImageResource(Resource.Drawable.Fail2);

                    break;
                case 5:
                    Hangman_image.SetImageResource(Resource.Drawable.Fail3);
                    break;

                case 4:
                    Hangman_image.SetImageResource(Resource.Drawable.Fail4);

                    break;

                case 3:
                    Hangman_image.SetImageResource(Resource.Drawable.Fail5);

                    break;

                case 2:
                    Hangman_image.SetImageResource(Resource.Drawable.Fail6);

                    break;

                case 1:
                    Hangman_image.SetImageResource(Resource.Drawable.Fail7);

                    break;

                // Case 0(0 turns left), the player has lost the game.  
                case 0:
                    Hangman_image.SetImageResource(Resource.Drawable.Fail8);
        ...
        private void Load_blank_Image()
        {
            ...
            Hangman_image.SetImageResource(Resource.Drawable.blankscreen);
        }
